...
// Step 2 → 3
btnProsseguirPix && btnProsseguirPix.addEventListener('click', () => {
...
function validateBriefing() {
  ...
  // Validação WhatsApp — deve ter pelo menos 10 dígitos
...
(function initDragDrop() {
  ...
  uploadArea.addEventListener('drop', e => {
    ...
    if (!file) return;
    // Reutiliza o input para disparar o change
...
function finalizarPedido() {
  // Monta resumo visual no step de sucesso
  ...
  if (resumoEl) {
    resumoEl.innerHTML = `
      ...
      <b>Comprovante:</b> ${comprovanteFile.name}
    `;
  }
